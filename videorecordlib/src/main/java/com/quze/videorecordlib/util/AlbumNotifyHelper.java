 ...
 * Created by mrtrying on 2018/8/16 17:12.
...
public class AlbumNotifyHelper {
     ...
     * @param height     高度
     */
    public static void insertVideoToMediaStore(Context context, String filePath, long createTime, int width, int height, long duration,double videoLatitude,double videoLongitude) {
        if (!checkFile(filePath))
            return;
        createTime = getTimeWrap(createTime);
        ContentValues values = initCommonContentValues(filePath, createTime);
        ...
        values.put(MediaStore.Video.Media.LATITUDE,videoLatitude);
        values.put(MediaStore.Video.Media.LONGITUDE,videoLongitude);
