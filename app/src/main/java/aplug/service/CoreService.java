...
public class CoreService extends NormalService {
	...
	public static final String xmlFile_wake = "xmlFile_wake";
	public static final String xmlKey_firstStart = "xmlKey_firstStart";
	public static final String ACTION_KEEPSERVICE = "ACTION_KEEPSERVICE";
	...
	private void start(String key, String value) {
		//获取时间点的最近启动日期
		String time = FileManager.loadShared(getApplicationContext(), xmlFile_wake, key).toString();
		//判断今天是否开启过 && 是否符合随机时间
		if (!value.equals(time) && isOpenActivityByRandom(getApplicationContext())) {
			//记录
			FileManager.saveShared(getApplicationContext(), xmlFile_wake, key, value);
	...
	private boolean isOpenActivityByRandom(Context context) {
		String timeStr = FileManager.loadShared(context, FileManager.xmlFile_appInfo, xmlKey_firstStart).toString();
	...
	private void appFirstStart(Context context) {
		...
		if (TextUtils.isEmpty(timeStr)) {
			timeStr = String.valueOf(System.currentTimeMillis());
			FileManager.saveShared(context, FileManager.xmlFile_appInfo, xmlKey_firstStart, timeStr);
		}
	}
