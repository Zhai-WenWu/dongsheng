...
public class ImageSelectorFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.image_seletor_fragment_multi_image, container, false);
    }
...
//                final int columnSpace = getResources().getDimensionPixelOffset(R.dimen.dp_2);
                final int columnSpace = Tools.getDimen(getActivity(), R.dimen.dp_3);
...
//                }
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (index == 0) {
                            getActivity().getLoaderManager().restartLoader(LOADER_ALL, null, mLoaderCallback);
                            mCategoryText.setText(R.string.folder_all);
                            if (mIsShowCamera) {
    ...
    public void onActivityCreated( Bundle savedInstanceState) {
        ...
        getActivity().getLoaderManager().initLoader(LOADER_ALL, null, mLoaderCallback);
    }
                ...
                if(currentArray != null && currentArray.size()>0){
                    ...
                    if (mCallback != null) {
                    	for(String path : currentArray){
                    		mCallback.onImageSelected(path);
                    	}
                    	for(String path : resultList){
                    		if(!currentArray.contains(path)){
                    			mCallback.onImageUnselected(path);
                    		}
                    	}
                    }
...
//                }
    ...
    private void selectImageFromGrid(AdapterView<?> adapterView,int position, int mode,View view) {
        ...
        if(image != null) {
            // 多选模式
            if(mode == ImageSelectorConstant.MODE_MULTI) {
        		Intent intent = new Intent(getActivity(),ImgWallActivity.class);
            	...
            	intent.putExtra(ImageSelectorConstant.EXTRA_SELECT_COUNT, mDesireImageCount);
            	 // 设定默认选择
                if(resultList != null && resultList.size()>0){
