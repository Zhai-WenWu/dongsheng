	...
	private void initData(){
		...
		this.registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {
			...
			public void onActivityResumed(Activity activity) {
				//记录当前activity
				if(!XHActivityManager.getInstance().getCurrentActivity().getComponentName().getClassName().equals(activity.getComponentName().getClassName())){
					XHActivityManager.getInstance().setCurrentActivity(activity);
				}
...
//		测试阶段建议设置成true，发布时设置为false
	...
	public XHAppActivityLifecycleCallbacks getXhAppActivityLifecycleCallbacks() {
		if(xhAppActivityLifecycleCallbacks!=null)
			return xhAppActivityLifecycleCallbacks;
		else return null;
	}

	public void setXhAppActivityLifecycleCallbacks(XHAppActivityLifecycleCallbacks xhAppActivityLifecycleCallbacks) {
		this.xhAppActivityLifecycleCallbacks = xhAppActivityLifecycleCallbacks;
	}
}
