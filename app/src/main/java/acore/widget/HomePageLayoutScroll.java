 ...
 * 
 */
public class HomePageLayoutScroll extends LinearLayout implements OnGestureListener {
	...
	public HomePageLayoutScroll(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public void animatScroll(int dx, int dy, int time) {
		if (scroller.isFinished()) {
			scroller.startScroll(scroller.getCurrX(), scroller.getCurrY(), dx, dy - scroller.getCurrY(), time);
			invalidate();
		}
	}
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (e1 != null && e2 != null) {
			...
			if (flag >= 0 && distanceY > 10) {
				...
				flag = -3;
				// if (scroller.getCurrY() == scrollHeight)
				// animatScroll(0, scrollHeight, 0, -scrollHeight, 1000);
			}
			if (flag < 0 && distanceY > 0)
				flag++;
			else if (flag > 0 && distanceY < 0)
				flag--;
		}
