...
public class MainMyself extends MainBaseActivity implements OnClickListener, IObserver {
    ...
    private String[] name1 = {"我的订单"},
            name2 = {"我的会员", "设备已开通会员，需绑定手机号","任务中心", "我的收藏", "浏览历史", "我的问答"},
            name3 = {"邀请好友", "反馈帮助","设置"};
    ...
    private void resetData() {
        ...
        scoreNum.setText("0");
        couponNum.setText("0");
        vipIcon.setVisibility(TextUtils.equals((CharSequence) FileManager.loadShared(this, FileManager.xmlFile_appInfo, "isShowVip"), "2") ? View.VISIBLE : View.GONE);
                    ...
                    if (!TextUtils.isEmpty(vip)) {
                        ...
                        if (vipArray.size() > 0) {
                            ...
                            if (TextUtils.isEmpty(vipData)) {
                                vipInfo.setVisibility(View.GONE);
                                if (vipNewHint.getVisibility() != View.VISIBLE)
                                    vipNewHint.setVisibility(View.VISIBLE);
                            } else {
            ...
            switch (index) {
                ...
                case R.id.header_bg:
                    if (DeviceVipManager.isDeviceVip()) {
                        ...
                        DeviceVipStatModel model = new DeviceVipStatModel("我的页面头部立即绑定_成功", "我的页面头部立即绑定_失败");
                        intent1.putExtra(DeviceVipStatModel.TAG, model);
    ...
    private void onListEventCommon(String clickTag) {
        ...
        if (!isOption) {
            switch (clickTag) {
                case "yiyuan"://权益迁移
                    if (LoginManager.isLogin()) {
                        DeviceVipStatModel model = new DeviceVipStatModel("已登录_我的页面会员下方立即绑定_成功", "已登录_我的页面会员下方立即绑定_失败");
                        DeviceVipManager.bindYiYuanVIP(MainMyself.this, model);
                    } else {
                        DeviceVipManager.setAutoBindDeviceVip(true);
                        Intent intent = new Intent(MainMyself.this, LoginByBindPhone.class);
                        DeviceVipStatModel model = new DeviceVipStatModel("未登录_我的页面会员下方立即绑定_成功", "未登录_我的页面会员下方立即绑定_失败");
                ...
                case "vip": //会员
                    ...
                    if (vipInfo.getVisibility() != View.VISIBLE)
