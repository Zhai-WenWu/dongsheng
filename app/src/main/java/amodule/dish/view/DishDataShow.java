...
public class DishDataShow extends DishBaseView {
    ...
    public DishDataShow(Context context, AttributeSet attrs) {
        super(context, attrs, R.layout.view_dish_data_show);
    }

    public DishDataShow(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        view_dish_header_data
        //主料
        ArrayList<Map<String, String>> burdenList = UtilString.getListMapByJson(lists.get(0).get("burden"));
        final ArrayList<Map<String, String>> zhuList= new ArrayList<Map<String, String>>();
        for (int i = 0, size = burdenList.size(); i < size; i++) {
           ArrayList<Map<String, String>> zhuList_1 = UtilString.getListMapByJson(burdenList.get(i).get("data"));
            ...
            if (zhuList.size() > 0) {
                ...
                for (Map<String, String> map : zhuList) {
                    map.put("goIngre", map.get("code").length() > 0 ? "ignore" : "hide");
                }
                AdapterSimple adapter = new AdapterSimple(zhuLiaoTab, zhuList,
                ...
                SetDataView.view(zhuLiaoTab, 1, adapter, null,
                        new SetDataView.ClickFunc[]{new SetDataView.ClickFunc() {
                            ...
                            public void click(int index, View v) {
                                Map<String, String> ingre = zhuList.get(index);
                                if (ingre.get("code").length() > 0) {
                                    XHClick.mapStat(context, tongjiId, "菜谱区域的点击", "用料部分的点击量");
                                    XHClick.mapStat(context, tongjiId, "食材部分的点击量", "");
                                    Intent intent = new Intent(context, DetailIngre.class);
                                    // 为了让搜索页搜code,显示Code
...
//            layoutParams.s
            ...
            recommendAd_linear.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    AppCommon.openUrl(activity,list_recommend.get(0).get("link"),true);
                    XHClick.mapStat(activity, tongjiId, "广告/运营", "用料下方广告位");
                }
            });
        }else{
            recommendAd_linear.setVisibility(View.GONE);
